...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Языки - en-US (основной), en (с приоритетом 0.9)
   - Серверу сообщается название и версия браузера (Mozilla) и движка (Gecko), а также ОС и архитектура (Ubuntu Linux x86_64)
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - мой IP: 192.168.0.108
   - IP сервера: 34.107.221.82
4. Какой код состояния возвращается с сервера на ваш браузер?
   - Для основного запроса - 200 OK (еще 301 Moved Permanently на запрос иконки favicon.ico)
...
<img width="1654" height="1037" alt="Задание 1" src="https://github.com/user-attachments/assets/1539a95d-249f-437f-b9de-e5aef6a78359" />
...
1. Проверьте содержимое первого HTTP-запроса GET. Видите ли вы строку «IF-MODIFIED-SINCE» в HTTP GET?
   - нет, такой строки нету, потому что это первый запрос
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - да, сервер вернул файл целиком
   - это можно увидеть либо по коду возврата (200 OK), либо по размеру payload'а (371 байт)
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - За IF-MODIFIED-SINCE следует время первого запроса (когда файл скачивался в последний раз)
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - Возвращается код 304 Not Modified (не изменено)
   - Никаких html-файлов сервер не выдал (нет тела запроса), так как файл на сервере не изменился с последнего скачивания

<img width="1654" height="1037" alt="Задание 2" src="https://github.com/user-attachments/assets/46ea2c00-aa68-4b4c-851b-36a9245f2aa7" />
...
1. Сколько сообщений HTTP GET отправил ваш браузер? Какой номер пакета в трассировке
   ...
   - было 2 запроса GET, первый на файл, второй на иконку
   - основной запрос имеет номер 12461
...
3. Сколько сегментов TCP, содержащих данные, потребовалось для передачи одного HTTP ответа?
   - 3 сегмента, два размером 1448 байт и третий 1968 байт
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - Нет, HTTP находится на более высоком уровне, поэтому не знает никакой информации о сегментах TCP (транспортный уровень)

<img width="1654" height="1037" alt="Задание 3" src="https://github.com/user-attachments/assets/3bf19d85-2871-403f-a1e0-eac5aad3737e" />
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - Если не считать favicon.ico и аутентификации вначале, было отправлено 3 запроса
   - Первый на изначальный адрес gaia.cs.umass.edu за указанным http-файлом
   - Новый запрос на gaia.cs.umass.edu за картинкой pearson.png
   - Запрос на kurose.cslash.net за второй картинкой
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - В данном случае изображения могли бы быть получены параллельно, потому что одно из них находится на другом сайте, соответственно, был открыт новый порт, который может работать одновременно с первым. Однако, если проверить Flow Graph в Wireshark (см. скрин), можно увидеть, что в данном случае запрос за второй картинкой начался после ответа с первой, так что конкретно здесь все было последовательно.
  
<img width="1580" height="384" alt="Задание 4" src="https://github.com/user-attachments/assets/ccdcf444-04a7-47de-83ea-a90b5165699d" />
...
1. Каков ответ сервера (код состояния и фраза) в ответ на начальное HTTP-сообщение GET от вашего браузера?
   - 401 Unauthorized (сервер защищен паролем, и пользователь не указал соответствующие поля)
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - Появляется поле Authorization. В нем указан тип авторизации, а на следующей строчке логин и пароль в незашифрованном виде, разделенные двоеточием

<img width="1387" height="1038" alt="Задание 5" src="https://github.com/user-attachments/assets/d54d2c88-6c60-4aa7-a65d-0cb2c22559af" />
